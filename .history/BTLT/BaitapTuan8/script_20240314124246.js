document.addEventListener(
  "DOMContentLoaded",
  function () {
    // Lúc trang web nạp vào
    document.getElementById("masv").focus(); // Di chuyển con trỏ đến ô nhập mã sản phẩm
    document.querySelector(
      "form button[type='submit']"
    ).style.opacity = "0.5";

    // Khi chọn dropdown Hình
    document
      .getElementById("img")
      ...
      .addEventListener(
        ...
        function (event) {
          ...
          cell2.innerHTML =
            '<img src="' +
            selectedImage +
            '.jpg" alt="Hình sản phẩm">'; // Hiển thị hình ảnh trong bảng phía dưới
          cell2.innerHTML +=
            "<br>Giá: " + selectedPrice;
          ...
          document.getElementById(
            "image-container"
          ).style.display = "none"; // Ẩn khung hình ảnh
          this.reset(); // Reset form
          masvInput.focus(); // Di chuyển con trỏ đến ô nhập mã sản phẩm
          document.querySelector(
            "form button[type='submit']"
          ).disabled = true; // Vô hiệu hóa nút Chọn
        }
      );
    ...
    var priceInput =
      document.getElementById("price");
    [masvInput, priceInput].forEach(function (
      input
    ) {
      input.addEventListener(
        ...
        function () {
          ...
          submitButton.disabled =
            isMasvEmpty ||
            isPriceEmpty ||
            !isImgSelected; // Nếu ô mã sản phẩm hoặc giá trống hoặc không có hình ảnh được chọn, vô hiệu hóa nút Chọn
        }
      );
    });
  }
